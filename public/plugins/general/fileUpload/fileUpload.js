...
var uploadTools = {
  ...
  "getInitOption":function(uploadId){
    ...
    var initOption={
      ...
      "onUpload":function(opt,data){//在上传之后
        // uploadTools.isUploaded = true;
        // uploadTools.uploadedList = data.imageUrl;
        // bootbox.alert('全部上传成功！');
      }
  ...
  "initWithSelectFile":function(opt){
    ...
    $("#"+uploadId+" .uploadBts .selectFileBt").on("click",function(){
      if(!opt.ismultiple && opt.fileList.length > 0){
        bootbox.alert('不能上传多个文件');
        return false;
      }
  ...
  "addFileList":function(fileList,opt){
    ...
    if(fileNumber+fileList.length>opt.maxFileNumber){
      bootbox.alert("最多只能上传"+opt.maxFileNumber+"个文件");
      return;
    }
    ...
    for(var i=0;i<fileList.length;i++){
      ...
      if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
        ...
        bootbox.alert("不支持该格式文件上传:"+fileList[i].name);
      }
    }
...
var uploadEvent = {
  ...
  "deleteFileEvent":function(opt,obj){
    ...
    var fileListArray = uploadFileList.getFileList(opt);
    var fileName = fileListArray[fileCodeId].name;
    delete fileListArray[fileCodeId];
    uploadFileList.setFileList(fileListArray,opt);
    fileItem.remove();
    if(opt.deleteFileUrl !== '#'){
      $.ajax({
        url: opt.deleteFileUrl + fileName,
        type: 'delete',
        dataType: 'json',
  ...
  "cleanFileEvent":function(opt){
    ...
    if(opt.deleteFileUrl !== '#'){
      $.each(fileList, function (index, file) {
        $.ajax({
          url: opt.deleteFileUrl + file.name,
          type: 'delete',
          dataType: 'json',
          success: function(res){
            if(res.err){
